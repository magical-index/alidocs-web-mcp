...
const MAX_FRAME_PAYLOAD = 8 * 1024 * 1024;
/** 分片重组后单消息最大长度 */
 ...
 * @param {{ mask?: boolean, fin?: boolean }} [options] mask 仅客户端方向需要
 * @returns {Buffer}
 */
function encodeFrame(opcode, payload, options) {
  ...
  const fin = opts.fin === undefined ? true : !!opts.fin;
 ...
 *
 * @param {{
 *   onMessage: (data: { opcode: number, payload: Buffer }) => void,
...
function createFrameParser(handlers) {
  ...
  let fragments = null; // { opcode, chunks: Buffer[], size: number }
  let failed = false;

  function fail(message, closeCode) {
    if (failed) return;
    failed = true;
    handlers.onError(new FrameError(message, closeCode));
  }

  /** @returns {boolean} 是否消费了一帧（false = 数据不足，等下次 push） */
  function tryParseOne() {
    ...
    for (let i = 0; i < payloadLength; i++) {
      payload[i] = maskKey ? raw[i] ^ maskKey[i % 4] : raw[i];
    }
